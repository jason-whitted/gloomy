...
class RoadEventsDialog extends Component {
  componentWillMount() {
    const { build } = this.props;
    const events = Array(69)
      .fill(0)
      .map((_, i) => (build.roadEvents.includes(i + 1) ? i + 1 : ''));
  ...
  submit = values => {
    this.props.onSubmit(build => ({
      ...build,
      roadEvents: values.events.filter(v => v).map(Number),
    }));
  };
    ...
    return (
      <Modal isOpen toggle={this.cancel}>
        <form onSubmit={handleSubmit(this.submit)}>
          <ModalHeader toggle={this.cancel}>Road Events</ModalHeader>
          <ModalBody>
            <Alert color="info">
              Mark the road events that are in your current event deck.
              <ul className="mb-0">
                <li>
                  ...
                  return (
                    <Field
                      ...
                      component={({ input }) => {
                        ...
                        const title = `Road Event ${i + 1}: ${input.value ? 'Available' : 'Unavailable/Removed'}`;
                        return (
                          <Button
                            type="button"
                            className={className}
...
export default reduxForm(formConfig)(RoadEventsDialog);
